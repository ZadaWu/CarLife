...
const extra = [...declared].filter(
  (k) => !known.has(k) && !/^(GATEWAY_PORT|AGENT_RUNTIME_PORT|AGENT_RUNTIME_URL|CARLIFE_GATEWAY_URL|CARLIFE_DEMO_TOKEN|CARLIFE_ASR_FAKE_TEXT|CARLIFE_TTS_SAY_VOICE|CARLIFE_AEC_ENABLED|CARLIFE_AEC_DELAY_MS|CARLIFE_SENTINEL_DEBUG|CARLIFE_CONFIG_TTL_MS|CARLIFE_AGENT_RUNTIME|CARLIFE_ANSWER_RUNTIME|CARLIFE_ANSWER_MODEL|CARLIFE_TITLE_MODEL|CARLIFE_PROBE_RATE_LIMIT|TAURI_APPLE_DEVELOPMENT_TEAM|TEST_DATABASE_URL)$/.test(k),
);
if (extra.length > 0) {
