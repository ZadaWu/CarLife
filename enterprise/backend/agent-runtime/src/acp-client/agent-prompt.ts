...
import { HIGHLIGHT_PROMPT } from "../llm/answer-format";
...
/*
 * 会话身份 → 规范 Agent 名，实现已搬进 `@carlife/acp`（M85-09 步 5）。
 *
 * 它是 ACP 的**会话命名约定**，不是车主面的策略：两个应用得答得一样，
 * 答得不一样时 prompt 文件、工具 ACL、进程池三处会各认各的名字。
 * 后缀家族与它踩过的坑写在 `shared/acp/src/naming.ts` 的文档注释里。
 */
export { canonicalAgent } from "@carlife/acp";
...
export function loadAgentPrompt(agent: AgentName | string): string {
   ...
   */
  if (/-(task|intent)$/.test(agent)) return raw;
  /*
   * 关键信息标注规则排在**业务 prompt 之后**：它讲的是"说出来的那段话怎么排版"，
   * 放在职责说明前面会被读成这一轮的首要任务。规则本体在 `llm/answer-format.ts`，
   * 叙述者（`NARRATOR_SYSTEM`）拼的是同一份常量——两条路的排版必须一样，
   * 否则同一个助手在不同话题下长得不一样。
   */
