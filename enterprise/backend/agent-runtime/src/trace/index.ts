...
export type TraceKind =
  ...
  | "risk"
  | "route"
  /** 观察层（M71-04）：照片 → 受控观察 + 图标匹配的结果与耗时——回放里要能看到「这轮看图看到了什么」。 */
  | "vision"
  /** 视频（M80-02）：网关派生的帧序图张数、转写段数、截断与缺失——回放里要能看到「这轮看视频看到了什么」。 */
  | "video"
  /** 表述模型这一步实际附了几张图（M80-02）——"图片到底有没有到模型"只能从这里看。 */
  | "media"
  | "agent_session"
  | "branch"
  | "merge"
  /** 行程可执行性体检（M77-03）：结论计数与修复轮数——回放里要能看到"这份方案验了几项、修了几处"。 */
  | "audit"
  | "tool_call"
  | "guard"
  | "interrupt"
