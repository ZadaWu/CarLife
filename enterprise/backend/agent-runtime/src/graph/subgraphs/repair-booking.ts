...
const BOOKING_RE =
  ...
  //
  // 「门店检查 / 到店检查 / 上门检查」是 M104 加的（2026-09-18 真跑 turn-81976fd8）：
  // 报告页「预约门店检查」按钮发的那句里只有「检查」没有「检修」，一字之差整条链走不到，
  // 车主看到的是「预约这块我这次没查到」。**只加带「门店 / 到店 / 上门」的组合词，不加裸「检查」**——
  // 裸的会把「帮我检查一下胎压」也拖进下单流程。
  /(预约|约个|约一下|约下|帮我约|想约).{0,30}(保养|维修|修车|进厂|检修|[门到上][店店门]检查)|(保养|维修|修车).{0,8}(预约|约个|约一下|约个时间)/;
