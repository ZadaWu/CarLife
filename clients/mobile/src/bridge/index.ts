 ...
 *
 * 与 cockpit 的同名文件**逐字相同**（本注释块除外，`test/bridge-parity.test.ts` 钉住）：
 * 两端消费的是同一份 `BRIDGE_EVENTS` 常量与同一套 `@carlife/shared` 类型，差异全在上层的
 * 屏幕组件里。抽进 clients/shared/ui 是正确方向，但那要先决定 UI 包该不该依赖 @tauri-apps/api——
 * 它目前不依赖，而一旦依赖，clients/shared/ui 就没法在纯浏览器（enterprise/console）里用了。
 * M65-02 之前两份已经漂开（手机端少了 permission / title / branch 三条订阅）。
 */
