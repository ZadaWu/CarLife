 ...
 *
 * 车机 `App.tsx` 里原本给「新建对话」按钮加的是丢弃式的闸（在飞就 `return`）。
 ...
 *
 * 手机端只有这一个 key——它没有「新建对话」入口与「退下」按钮（那两个是车机的 HUD 才有）。
 * **与车机各存一份而不是抽公共包**：两端的引导结构不同（车机有 `canResume`、
 * 哨兵绑定、会话列表），跨 app 共享代码属结构变更，要先立 ACR。
 */
export const INFLIGHT_BOOTSTRAP = "session:bootstrap";
/** 第一次说话时现建会话（M50-02 之后手机端唯一会建出会话的路径）。 */
